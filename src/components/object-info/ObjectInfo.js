  ...
  return (
    <section className={`pa4 sans-serif ${className}`} {...props}>
      <h2 className="ma0 lh-title f4 fw4 montserrat pb2" title={type}>
        ...
        <span className="v-mid">{nameForNode(type)}</span>
        {format === 'unixfs' ? (
          <a
            className="dn di-ns link charcoal ml2"
            href="https://docs.ipfs.io/guides/concepts/unixfs/"
          >
            UnixFS
          </a>
        ) : null}
        {format === 'unixfs' && data.type && ['directory', 'file'].some((x) => x === data.type) ? (
          <a
            className="link avenir ml2 pa2 fw5 f6 blue"
      ...
      <div className="f6">
        {!cid ? null : (
          <div className="dt dt--fixed pt2">
            <label className="dtc silver tracked ttu f7" style={{ width: 48 }}>
              CID
            </label>
            <div className="dtc truncate charcoal monospace" data-id="ObjectInfo-cid">
              {cid}
            </div>
          </div>
        )}
        ...
        <div className="dt dt--fixed pt2" style={{ height: 26 }}>
          <label className="dtc silver tracked ttu f7 v-mid" style={{ width: 48 }}>
            Data
          </label>
          <div className="dtc truncate mid-gray">{data ? null : 'No data'}</div>
        </div>
        {!data ? null : (
